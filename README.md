...
父仓库只记录子模块*指向哪个提交*——所以父仓库既显示不了它们的文件，也没法在它们里面提交。
两个工具栏最左边的仓库控件就是让它们可达的东西：选中一个，视图背后的每条命令（状态、diff、日志、分支、储藏、提交、hunk 暂存）
就都在它里面跑。
...
- **「Copy Hash」菜单**复制的是文件路径，但标签写的是 Hash——标签与行为不一致，属于待修项。
