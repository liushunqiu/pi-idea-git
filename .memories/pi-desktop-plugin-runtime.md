...
## 5j. "半选"状态：先证明它可达，再决定画什么

**症状**：把扁平变更列表改成目录树、打算给文件夹加三态复选框时，`indeterminate`
**永远不出现**——但代码看起来完全正确。

**根因（两层）**：
1. **结构上不可达**：UI 每个分组本身就是一种状态（"已暂存"组里每个文件都在索引中，
   "未暂存"组里每个文件都不在）。勾选一个文件会让它**换到另一个分组**，而不是在原
   地留下"半个勾"的文件夹。所以在同一分组内，"部分子文件已勾选"这个状态
   **不可能存在**——`some && !all` 恒为假。加多少断言都测不出一个不可达的状态。
2. **被自己的守卫压掉**：第一版写 `indeterminate = partial && !checked`，而唯一
   接近"半"的情形（文件夹整体已纳入、但其中有 `MM` 文件）恰好是 `partial && checked`，
   被这行守卫精确过滤掉。

**最终结论：这个状态不该被标记，更不该用三态控件。**

用户看到实现后一句话点破：*"怎么会有存在部分已暂存的状态呢？这很奇怪的啊，都是同一个
文件"*。他说得对——**同一文件同时出现在"已暂存"和"未暂存"两个分组里，就已经是这件事
的完整表达**：

- `git status --porcelain=v2` 报 `MM`（索引改了 + 工作区又改了）或 `AM`（新文件已 add
  又改过），两个列表各有一条记录，各带自己的状态字母；
- VS Code 把它列在 `Staged Changes` **和** `Changes` 下；
- IDEA 把它同时放进 Staged / Unstaged 两个 changelist 分组。

三者都不给文件贴第二个标签。我先后加过的 **`indeterminate` 半选**和 **"部分已暂存"
徽标**，都是**发明词汇**：用户拿它跟 `git status` 对不上账，只会更困惑。
最终全部删除（含 `.badge.partial`、两条中英文案、以及那个 `partial` 字段）。

**通用教训（三条）**：
1. **加三态/多态之前，先用数据证明那个状态能被构造出来**。一个恒不可达的分支配上
   "我加过判断了"的自信，比没有这个分支更难查（`some && !all` 在同一分组内恒为假）。
2. **状态该不该显示，先问"Git 自己怎么表达它"**。凡是 `git status` 里没有的概念，
   界面上就不要发明一个词去描述它——用户会拿两处对账，对不上就认定是 bug。
3. **把状态搬到 UI 上时，先确认控件的语义与实际可达状态一一对应**。对不上时宁可用
   二态控件 + 不加说明，也不要让一个"看起来像部分选中"的成态去承担别的含义。

**测试方法上的次生教训**：`paintChanges()` 是整块重建 DOM，
**跨重绘持有旧节点去读属性，量到的是已 detach 的元素**——我那版 CDP 探针因此报出
"dash 永远不出现"的假结论，白白多绕一轮。每次交互后必须重新 `querySelectorAll`。
