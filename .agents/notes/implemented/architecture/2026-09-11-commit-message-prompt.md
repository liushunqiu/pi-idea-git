# Agent Note: 提交信息生成：把语言从"推断"改成参数
...
- **收益：语言是显式参数，不再由仓库历史单方面决定。** 生成按钮的 chevron
  菜单提供 `auto` / 简体中文 / 英文（随 `git/prefs-set` 持久化），显式选择压过
  仓库历史，`auto` 在无历史时退回界面语言——中文用户第一次有了让提示词
  "知道用户要什么语言"的位置。
- **收益：仓库惯例由插件测量，不让模型猜。** `commitStyle()` 从最近 40 条完整
  提交信息量出历史语言、`type(scope):` 前缀惯例（≥2 条命中才算）与是否写正文；
  不用的仓库不会被无条件套上 `feat:` 前缀或"写正文"的惯例。
- **收益：机械可判的格式由代码强制成立。** `formatCommitMessage(raw)` 保证
  subject 后恒有空行——"消息框看着像两段、实际是一条巨长标题"这类只有
  `git log` 才暴露的失败被消除；超长标题在子句边界下沉而非截断，正文按显示
  宽度折到 72 列，中文之间的半角 `,` `;` 归一到 `，` `；`，且幂等。
- **收益：两条主线都有可执行证据。** 语言侧 18 种组合 0 处矛盾
  （`lang-matrix.mjs`）、`lang-probe.mjs` 24/24、`user-message-probe.mjs`
  14/14；格式侧 `format-probe.mjs` 10 个用例通过全部不变量，回归 harness
  51/51 通过。
- **代价：默认仍是 `auto`，选择权交出去也意味着用户要做一次决定。** 保留旧
  行为、不惊吓老用户的代价，是英文历史仓库 + 中文界面下不显式选择就仍是
  英文草稿。
- **代价：提示词只提高概率，格式保证落在代码层。** system 文本按语言与 CC
  惯例分支并附带填写示例；"After the subject, leave a blank line and then the
  body" 早就在提示词里，模型照样不遵守。`formatCommitMessage` 这层确定性重整
  自身也有实现成本：第一版无条件补空行会改坏已正确的草稿，折行续行会被重复
  折行（解法："只在真有空行处断段"与 `joinWrapped`）。
- **已知上限：一处前缀不一致未处理。** `--no-prefix` 只用于提示词、不回喂
  `git apply`；文件级草稿走 `readDiff()` 仍带 `a/` `b/` 前缀。
- **已知上限：提示词措辞没有单元测试可断言**，证据只来自探针；"测不出来"
  必须以 `null` 流到调用方且各成因互斥——把"平局"（可测量）误算进"未知"曾让
  "中文 CC 仓库 + 英文界面"退回英文（RV-001，已修）。
- **重访信号：** 再出现"提示词写了但模型不遵守"的机械可判格式失败时，按同一
  结论继续在代码层强制；新判据凡落到 `null`/默认值，先给每个成因一个真实仓库
  例子（发布前自检）。
